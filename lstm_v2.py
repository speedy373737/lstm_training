...
# Konstanten
NUM_PRODUCT_GROUPS = 1  # nur die erste Produktgruppe wird verwendet
...
# Filtere nur die erste Produktgruppe (Gruppe == 0)
train_df = train_df[train_df['Gruppe'] == 0]
val_df = val_df[val_df['Gruppe'] == 0]
test_df = test_df[test_df['Gruppe'] == 0]
...
def build_model(hp):
    ...
    embedding_size = hp.Int("embedding_size", min_value=4, max_value=16, step=2)
    learning_rate = hp.Float("learning_rate", min_value=1e-4, max_value=1e-2, sampling="LOG")

    # Inputs: volle Zeitreihe wird genutzt
    ...
    # LSTM-Zweig
    x = layers.LSTM(lstm_units, dropout=dropout_rate)(timeseries_input)
...
tuner = BayesianOptimization(
    ...
    directory='lstm_bayes_dir_v2',
    project_name='lstm_bayes_opt_v2'
)
